# Termo de Abertura de Projeto Interno

**Nome do Projeto:** [Nome do projeto]  
**Tipo:** [ ] Pesquisa  [ ] Extensão  [ ] Ensino  [ ] Misto  
**Período:** [Data de início] a [Data de término prevista]  
**Versão:** 1.0  
**Data:** [DD/MM/AAAA]

#### Notas do que joao disse
- Construir todo o framework em c para comunicação wren -> hardware
- Usar a api do wren para contruir o framework em c
- Depois de validar isso funcionando, construir uma extensão tipo platformio para usar esse framework

---

## 1. Identificação do Projeto

### 1.1 Título do Projeto
[Nome completo e descritivo do projeto]

### 1.2 Líder do Projeto
**Nome:** [Nome completo]  
**Contato:** [E-mail e telefone]  
**Cargo na LASER:** [Membro/Coordenador/etc]  
**Área de Especialização:** [Área técnica]

### 1.3 Vice-Líder (se aplicável)
**Nome:** [Nome completo]  
**Contato:** [E-mail e telefone]

---

## 2. Justificativa

### 2.1 Contexto e Motivação
[Descrever o contexto que motivou a criação do projeto, o problema identificado ou a oportunidade detectada]

### 2.2 Relevância Acadêmica
[Explicar a importância do projeto para a formação dos membros e para o avanço do conhecimento na área]

### 2.3 Impacto Social e Extensão
[Descrever como o projeto pode beneficiar a comunidade externa, empresas, organizações ou a sociedade em geral]

### 2.4 Alinhamento com os ODS
Objetivos de Desenvolvimento Sustentável relacionados:
- [ ] ODS 4 - Educação de Qualidade
- [ ] ODS 9 - Indústria, Inovação e Infraestrutura
- [ ] ODS 11 - Cidades e Comunidades Sustentáveis
- [ ] ODS 17 - Parcerias e Meios de Implementação
- [ ] Outros: _________________

### 2.5 Conexão com Áreas Estratégicas da LASER
- [ ] IoT
- [ ] Automação Residencial e Predial
- [ ] Robótica Aplicada
- [ ] Agricultura de Precisão
- [ ] Tecnologias para Saúde
- [ ] Engenharia Biomédica
- [ ] IA em Sistemas Embarcados
- [ ] Tecnologias Automotivas
- [ ] Sistemas Críticos
- [ ] Processamento de Sinais
- [ ] Outras: _________________

---

## 3. Objetivos

### 3.1 Objetivo Geral
[Objetivo principal e abrangente do projeto]

### 3.2 Objetivos Específicos
1. [Objetivo mensurável 1]
2. [Objetivo mensurável 2]
3. [Objetivo mensurável 3]
4. [Objetivo mensurável 4]

### 3.3 Resultados Esperados

#### 3.3.1 Produtos/Entregas
- [Entrega 1: ex: protótipo funcional]
- [Entrega 2: ex: documentação técnica]
- [Entrega 3: ex: artigo científico]

#### 3.3.2 Impactos Esperados
- **Acadêmico:** [Publicações, TCCs, apresentações]
- **Social:** [Beneficiários, alcance da extensão]
- **Técnico:** [Tecnologias desenvolvidas, inovações]
- **Institucional:** [Fortalecimento da LASER, parcerias]

---

## 4. Composição da Equipe

### 4.1 Equipe Principal

| Nome Completo | Curso/Período | Área de Expertise | Papel no Projeto | Carga Horária Semanal |
|---------------|---------------|-------------------|------------------|-----------------------|
| [Nome 1] | [Curso] | [Área] | [Papel] | [Horas] |
| [Nome 2] | [Curso] | [Área] | [Papel] | [Horas] |
| [Nome 3] | [Curso] | [Área] | [Papel] | [Horas] |

### 4.2 Estrutura Multidisciplinar
**Áreas representadas:**
- [ ] Ciência da Computação
- [ ] Engenharia da Computação
- [ ] Engenharia Eletrônica
- [ ] Engenharia Mecânica
- [ ] Design
- [ ] Administração
- [ ] Outras: _________________

### 4.3 Orientadores e Colaboradores Externos
| Nome | Vínculo | Papel | Tipo de Apoio |
|------|---------|-------|---------------|
| [Nome] | [Instituição] | Orientador | [Tipo] |
| [Nome] | [Empresa/Org] | Parceiro | [Tipo] |

---

## 5. Metodologia e Operacionalização

### 5.1 Abordagem Metodológica
[Descrever a metodologia geral: metodologia ágil, design thinking, pesquisa-ação, etc]

### 5.2 Fases do Projeto

#### Fase 1: [Nome da Fase]
**Período:** [Data início - Data fim]  
**Objetivos:** [Objetivos específicos desta fase]  
**Atividades:**
- [Atividade 1]
- [Atividade 2]
- [Atividade 3]

#### Fase 2: [Nome da Fase]
**Período:** [Data início - Data fim]  
**Objetivos:** [Objetivos específicos desta fase]  
**Atividades:**
- [Atividade 1]
- [Atividade 2]
- [Atividade 3]

#### Fase 3: [Nome da Fase]
**Período:** [Data início - Data fim]  
**Objetivos:** [Objetivos específicos desta fase]  
**Atividades:**
- [Atividade 1]
- [Atividade 2]
- [Atividade 3]

### 5.3 Ferramentas e Tecnologias
**Hardware:**
- [Equipamento/Plataforma 1]
- [Equipamento/Plataforma 2]

**Software:**
- [Ferramenta 1]
- [Ferramenta 2]

**Plataformas de Gestão:**
- [Ferramenta de gerenciamento de projeto]
- [Ferramenta de versionamento]
- [Ferramenta de documentação]

---

## 6. Escopo do Projeto

### 6.1 Dentro do Escopo
✅ [Item incluso 1]  
✅ [Item incluso 2]  
✅ [Item incluso 3]  

### 6.2 Fora do Escopo
❌ [Item excluído 1]  
❌ [Item excluído 2]  
❌ [Item excluído 3]  

### 6.3 Premissas
- [Premissa 1]
- [Premissa 2]
- [Premissa 3]

### 6.4 Restrições
- [Restrição 1: ex: orçamento]
- [Restrição 2: ex: prazo]
- [Restrição 3: ex: recursos disponíveis]

---

## 7. Cronograma Macro

| Fase/Marco | Descrição | Data Prevista | Responsável |
|------------|-----------|---------------|-------------|
| Início do Projeto | Kickoff e planejamento detalhado | | |
| Marco 1 | [Descrição do marco] | | |
| Marco 2 | [Descrição do marco] | | |
| Marco 3 | [Descrição do marco] | | |
| Entrega Final | Conclusão e apresentação | | |

---

## 8. Orçamento e Recursos

### 8.1 Recursos Financeiros

| Item | Descrição | Quantidade | Valor Unitário | Valor Total | Fonte de Recurso |
|------|-----------|------------|----------------|-------------|------------------|
| [Categoria 1] | [Detalhes] | | R$ | R$ | [Origem] |
| [Categoria 2] | [Detalhes] | | R$ | R$ | [Origem] |
| [Categoria 3] | [Detalhes] | | R$ | R$ | [Origem] |

**Orçamento Total:** R$ [valor]

### 8.2 Fontes de Financiamento
- [ ] Recursos próprios da LASER
- [ ] Patrocínio empresarial
- [ ] Edital de fomento
- [ ] Parceria institucional
- [ ] Crowdfunding
- [ ] Outros: _________________

### 8.3 Recursos Humanos
**Total de horas estimadas:** [número] horas  
**Membros envolvidos:** [número] pessoas  
**Carga horária média semanal por membro:** [número] horas

### 8.4 Infraestrutura
- [ ] Laboratório do CIn/UFPE
- [ ] Espaço próprio da LASER
- [ ] Infraestrutura de parceiros
- [ ] Equipamentos específicos necessários

---

## 9. Gestão de Riscos

| ID | Risco | Probabilidade | Impacto | Prioridade | Estratégia de Mitigação | Responsável |
|----|-------|---------------|---------|------------|-------------------------|-------------|
| R1 | [Descrição] | A/M/B | A/M/B | [Nível] | [Ação] | [Nome] |
| R2 | [Descrição] | A/M/B | A/M/B | [Nível] | [Ação] | [Nome] |
| R3 | [Descrição] | A/M/B | A/M/B | [Nível] | [Ação] | [Nome] |

**Legenda:**  
A = Alta | M = Média | B = Baixa

---

## 10. Plano de Comunicação e Divulgação

### 10.1 Comunicação Interna
**Reuniões da Equipe:**
- **Frequência:** [Periodicidade]
- **Formato:** [Presencial/Híbrido/Online]
- **Plataforma:** [Ferramenta]

**Ferramentas de Comunicação:**
- Comunicação assíncrona: [WhatsApp/Discord/Slack]
- Gestão de tarefas: [Trello/Asana/Notion]
- Repositório de código: [GitHub/GitLab]
- Documentação compartilhada: [Google Drive/Notion/Confluence]

### 10.2 Comunicação com a LASER
**Relatórios de Progresso:**
- **Frequência:** [Mensal/Bimestral]
- **Destinatário:** [Coordenação específica]
- **Formato:** [Documento/Apresentação]

### 10.3 Divulgação Externa
**Público-Alvo:** [Comunidade acadêmica/Empresas/Sociedade]

**Canais de Divulgação:**
- [ ] Redes sociais da LASER
- [ ] Website da LASER
- [ ] Eventos de extensão
- [ ] Publicações científicas
- [ ] Apresentações em congressos
- [ ] Workshops abertos
- [ ] Outros: _________________

**Responsável pela Divulgação:** [Nome]

---

## 11. Plano de Parcerias e Extensão

### 11.1 Parcerias Previstas
| Organização | Tipo | Natureza da Parceria | Status | Contato |
|-------------|------|---------------------|--------|---------|
| [Nome] | Empresa/ONG/Instituição | [Descrição] | [ ] Em negociação<br>[ ] Confirmada | [Nome/Email] |

### 11.2 Eventos de Extensão Planejados
| Evento | Tipo | Público-Alvo | Data Prevista | Responsável |
|--------|------|--------------|---------------|-------------|
| [Nome] | Workshop/Palestra/Curso | [Público] | [Data] | [Nome] |

### 11.3 Impacto na Comunidade Externa
**Estimativa de Alcance:**
- Número de pessoas diretamente beneficiadas: [número]
- Número de pessoas indiretamente impactadas: [número]
- Organizações envolvidas: [número]

---

## 12. Avaliação e Indicadores de Sucesso

### 12.1 Indicadores Quantitativos
| Indicador | Meta | Forma de Medição |
|-----------|------|------------------|
| [Indicador 1] | [Valor/Percentual] | [Método] |
| [Indicador 2] | [Valor/Percentual] | [Método] |
| [Indicador 3] | [Valor/Percentual] | [Método] |

### 12.2 Indicadores Qualitativos
- [Indicador qualitativo 1]
- [Indicador qualitativo 2]
- [Indicador qualitativo 3]

### 12.3 Metodologia de Avaliação
[Descrever como será feita a avaliação do projeto: reuniões de acompanhamento, relatórios, apresentações, etc]

### 12.4 Critérios de Sucesso
- [ ] Todos os objetivos específicos alcançados
- [ ] Entregas realizadas conforme cronograma
- [ ] Impacto mensurável na comunidade externa
- [ ] Satisfação dos membros da equipe
- [ ] Documentação completa e publicada
- [ ] [Critério específico adicional]

---

## 13. Sustentabilidade e Continuidade

### 13.1 Plano de Continuidade
[Descrever como o projeto poderá ter continuidade após o período inicial, incluindo possibilidade de novas fases, expansão ou incorporação às atividades regulares da LASER]

### 13.2 Transferência de Conhecimento
[Explicar como o conhecimento gerado será documentado e transferido para futuros membros da LASER]

### 13.3 Possibilidades de Evolução
- [Possibilidade 1]
- [Possibilidade 2]
- [Possibilidade 3]

---

## 14. Aprovações

### 14.1 Coordenador do Projeto
**Nome:** _____________________  
**Assinatura:** _____________________  
**Data:** ___/___/______

### 14.2 Coordenação Relacionada da LASER
**Cargo:** [ ] Pesquisa  [ ] Extensão e Ensino  [ ] Presidente  
**Nome:** _____________________  
**Assinatura:** _____________________  
**Data:** ___/___/______

### 14.3 Presidente da LASER
**Nome:** _____________________  
**Assinatura:** _____________________  
**Data:** ___/___/______

### 14.4 Orientador/Professor (se aplicável)
**Nome:** _____________________  
**Departamento:** _____________________  
**Assinatura:** _____________________  
**Data:** ___/___/______

---

## 15. Anexos

- [ ] Cronograma detalhado
- [ ] Orçamento detalhado
- [ ] Cartas de apoio/parceria
- [ ] Especificações técnicas
- [ ] Referencial teórico
- [ ] Outros documentos relevantes

---

## 16. Histórico de Revisões

| Versão | Data | Autor | Descrição das Alterações |
|--------|------|-------|--------------------------|
| 1.0 | [Data] | [Nome] | Versão inicial |
| | | | |

---

**Observações Adicionais:**
[Espaço para observações, considerações especiais ou informações complementares]
